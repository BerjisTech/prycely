      ...
      if user_signed_in?
        ...
        if @already_invited.nil?
          ...
          Member.new(invited_by: @invite.user_id, user_id: current_user.id, group_id: @invite.group_id,
                     designation: Designation.find_or_create_by(name: 'Member').id, status: '1', invited_on: Date.today, accepted_on: Date.today, paid_member: '', amount: 0, account_id: @account[0]).save
