...
class Log < ApplicationRecord
  ...
  class << self
    def mine(user_id, limit = 0, offset = 0)
      Log.where(user_id: user_id).limit(limit).offset(offset)
    end

    def for_group(wallet_id, limit = 0, offset = 0)
