...
class Wise < ApplicationRecord
  class << self
    def request_get(path, body = [], headers = [])
      data = Faraday.get(path, body, headers)

      data.body
    end

    def request_post(path, body = [], headers = [])
      Faraday.post(path, body.to_json, headers)
    end

    def business_details(app)
    ...
    def create_quote(app) # rubocop:disable Metrics/MethodLength
    ...
    def create_transfer(app) # rubocop:disable Metrics/MethodLength
    ...
    def create_recepient_account(app) # rubocop:disable Metrics/MethodLength
      path = "#{Api.wise_api(app).endpoint}/v1/accounts"
      headers = {
        Authorization: "Bearer #{ENV['WISE_TOKEN']}",
        'Content-Type': 'application/json'
